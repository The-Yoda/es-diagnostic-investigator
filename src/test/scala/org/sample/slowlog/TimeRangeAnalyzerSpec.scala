...
class TimeRangeAnalyzerSpec extends WordSpecLike with Matchers {
  ...
  "getMaxTimeRange" should {
    "get time range for given format" in {
      ...
      getMaxTimeRange(map(json), queriedTime) shouldBe 86399
    }
    "get max time range out of different ranges" in {
      ...
      getMaxTimeRange(map(json), queriedTime) shouldBe 86401
    }
    ...
    "get -1 when there is no range in the query" in {
      ...
      getMaxTimeRange(map(json), queriedTime) shouldBe -1
    }
  ...
  "getTimeRange" should {
    "get time range for default(UTC) format" in {
      getTimeRange(Map("gte" -> "2017-11-22T08:00:00.006Z", "lt" -> "2017-11-23T08:00:00.006Z"), queriedTime) shouldBe 86399
    }

    "get time range for given format" in {
      getTimeRange(Map("gte" -> "2017-11-22", "lte" -> "2017-11-23", "format" -> "yyyy-MM-dd"), queriedTime) shouldBe 172799
    }

    "get time range for epoch millis" in {
      getTimeRange(Map("lte" -> "1511456399999", "gte" -> "1511454600000"), queriedTime) shouldBe 1799
    }

    "get time range for given time zone" in {
      getTimeRange(Map("gte" -> "2017-11-22T08:00", "lte" -> "2017-11-23T08:00", "time_zone" -> "America/Los_Angeles"), queriedTime) shouldBe 86459
    }

    "get time range for given time zone with no upper bound" in {
      getTimeRange(Map("gte" -> "2017-11-23T08:00:00.006Z", "time_zone" -> "America/Los_Angeles"), queriedTime) shouldBe 32399
    }

    "get time range for given time zone with no lower bound" in {
      //should get time difference since year 2000 to upper bound
      getTimeRange(Map("lte" -> "2017-11-23T08:00:00.006Z", "time_zone" -> "America/Los_Angeles"), queriedTime) shouldBe 564739200
    }
  }

  private def map(json: String) = Map().fromJSON(json)
}
