  ...
  "SlowLogCruncher" should {
    "get time range for given format" in {
      ...
      val x = Source.single(slowLog).via(SlowLogCruncher.transform(DateTimeZone.UTC.getID))
        ...
        .request(1)
        .expectNext()
      x shouldBe Map("index" -> "log-2017-10-25", "latency" -> 17, "node" -> "machine1-data01", "query" -> """{"from":0,"query":{"filtered":{"filter":{"bool":{"must":[{"range":{"timestamp":{"gte":"2017-11-22T08:00:00.006Z","lt":"2017-11-23T08:00:00.006Z"}}}]}}}},"size":2000}""", "search_type" -> "QUERY_THEN_FETCH", "shard_id" -> "0", "signature" -> -948470321, "size" -> 165, "timerange" -> 86399, "timestamp" -> "2017-11-23T00:00:00.392Z", "total_shards" -> "2400")
    }
  }
}
